...
export class ProductService {
  ...
  getProducts(): Observable<Product[]> {
    return this.httpClient.get<Product[]>(
      this.url + '/products?pagination=false'
    );
  }
  ...
  getActiveProducts(): Observable<Product[]> {
    return this.httpClient.get<Product[]>(
      this.url + '/products?pagination=false&active=true'
    );
  }
  ...
  getProductsByCategoryId(categoryId: string): Observable<Product[]> {
    return this.httpClient.get<Product[]>(
      this.url + '/products/findByCategory/' + categoryId + '?active=true'
    ); //waiting for specific call
  }
  ...
  putProduct(id: string, product: Product): Observable<Product> {
    let headers = new HttpHeaders();
    headers = headers.set('Content-Type', 'application/json; charset=utf-8');
    return this.httpClient.put<Product>(this.url + '/products/' + id, product, {
      headers: headers,
    });
  }
